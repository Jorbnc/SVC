...
# This allows to use the physics package of MathJax and its LaTex macros. It's a LaTeX call.
md"""$\require{physics}$"""
...
$(y \leq x \land -y \leq x) \implies \lvert{y}\rvert \leq x$. Por lo tanto, 

!!! warning ""
	Sin pérdida de generalidad con respecto a cuál valor es el minuendo o sustraendo, tenemos que
...
let
	...
	anim = @animate for i in 0:0.1:Δ
		plot(x, f.(x), color = "black", line=2,
			xticks=([50], ["\$ x_0 \$"]), yticks=([50], ["\$ L \$"]),
			tickfontsize=15) # Main function y=f(x)
...
let
	...
	anim = @animate for i in 10:0.10:20
		fig = plot(layout = grid(1,2), legend=false, size=(655,250), tickfontsize=8)
		for j in 1:2
			...
			annotate!(fig[j], 22, -2500, text("\$ L-\\epsilon \$", :red, :left))
			annotate!(fig[j], 12, f(21), text("\$ x_0+\\delta \$", :blue, :bottom))
			annotate!(fig[j], -12, f(21), text("\$ x_0-\\delta \$", :blue,  :bottom))
		end
		vline!(fig[1], [-10, 10], color=:blue)
...
let
	...
	anim = @animate for i in 0:0.05:3
		plot(x3, h.(x3), color = "black", line=2, xticks=([2], ["\$ x_0 \$"]), 
			yticks=[], xlims=[-0.5, 3], label="f(x) by parts",
			framestyle=:origin, right_margin=12mm, tickfontsize=15)
...
Además, es necesario que $x \neq x_0$ porque $f(x_0)$ podría no estar definido ($x_0$ podría no estar en el dominio de $f(x)$). Entonces, 
...
let fig = plot(layout = grid(2,2), legend=false, size=(600,400), xticks=[], yticks=[],
	tickfontsize=15, bottom_margin=10mm)
	f(x) = x^2
	x1 = Vector(0:0.1:3.5)
...
md"""
## Límite infinito en un punto en x
Ejemplo:
"""
...
let
	...
	p = plot(x, f.(x), framestyle=:origin, line=2, xticks=([1], ["\$x_0\$"]), yticks=[],
		tickfontsize=15, xlims=[], ylims=[0, 10])
	vline!([1], line=(:dash))
	δ = 0.30
...
let f(x) = 1/(1+100^(-x))
	...
	p = plot(x, f.(x), framestyle=:origin, size=(350, 210), line=:2,
		xticks=[], yticks=([1], ["\$L\$"]), tickfontsize=15,
...
!!! warning ""
	...
	$\begin{align}
	\lim_{x \to x_0}\big[f(x)+g(x)\big] = \lim_{x \to x_0}f(x) + \lim_{x \to x_0}g(x)
	\end{align}$
...
md"""
Finalmente, sumando las inecuaciones (ambos consecuentes) tenemos
...
!!! warning ""
	$\begin{equation}
	\lim_{x \to 0}f(x)-g(x) = \lim_{x \to 0}f(x)-\lim_{x \to 0}g(x) \qquad \blacksquare
	\end{equation}$
"""
...
!!! warning ""
	...
	$\begin{align}
	\lim_{x \to x_0}\big[f(x)g(x)\big] = \lim_{x \to x_0}f(x) \times \lim_{x \to x_0}g(x)
	\end{align}$
...
!!! warning ""
	...
	$\begin{align}
	\lim_{x \to x_0}\frac{1}{f(x)} = \frac{1}{\displaystyle\lim_{x \to x_0}f(x)}
	\end{align}$
...
!!! warning ""
	$\lim_{x \to x_0}\frac{f(x)}{g(x)} = \frac{\displaystyle\lim_{x \to x_0}f(x)}
	{\displaystyle\lim_{x \to x_0}g(x)} \quad \blacksquare$
"""
...
!!! warning ""
	...
	$\begin{align}
	f(x)\leq g(x) \leq h(x) \; \land \; \lim_{x \to x_0}f(x) = \lim_{x \to x_0}h(x) = L \implies \lim_{x \to x_0}g(x) = L
	\end{align}$
...
- [milefoot.com] (http://www.milefoot.com/math/calculus/limits/GenericLimitLawProofs04.htm)
"""
